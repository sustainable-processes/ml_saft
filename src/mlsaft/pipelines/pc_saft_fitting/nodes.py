...
from mlsaft.extras.utils.data_transform import jsonify_dict
from mlsaft.extras.utils.pcsaft import is_associating, pcsaft_lmfit_regression
from mlsaft.extras.utils.sensitivity import pcsaft_sensitivity_analysis
from mlsaft.pipelines.train_models.models.spk import (
    ASEAtomsData,
    AtomsLoader,
    SPKTrainArgs,
    _atoms_collate_fn,
...
def parameter_dicts_to_dataframes(
    ...
    for batch_fn in batches.values():
        for result in batch_fn():
            ...
            check_max_residual = (
                max_residual and max_residual_batch < max_residual
            ) or not (max_residual)
