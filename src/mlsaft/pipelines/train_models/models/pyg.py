...
from .pcsaft_emulator import PcSaftEmulatorLightningModule
...
class MPNN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # special pooling
        dim_channel_in = self.args.dim_fingerprint  # vector dimension after pooling (does not change for add-, mean-, max-pooling)
    ...
    def forward(self, data):
        ...
        ### MLP-prediction ###
        if self.args.separate_heads:
            y = torch.cat([self.ffns[i](h) for i in range(self.num_targets)], axis=1)  # type: ignore
        else:
            y = self.ffn(h)
        return y
