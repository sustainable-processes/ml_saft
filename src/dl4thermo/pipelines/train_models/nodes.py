...
# Extras
from dl4thermo.extras.utils import (
    calculate_metrics,
...
from dl4thermo.extras.utils.doe import generate_search
from dl4thermo.extras.utils.lightning import load_lightningmodule_from_checkpoint
from dl4thermo.extras.utils.pcsaft import is_associating, predict_phase_diagram
from dl4thermo.extras.utils.wandb_utils import download_best_model  # type: ignore
...
def calculate_pcsaft_predictions(parameter_df: pd.DataFrame):
    ...
    mw = 100  # Molecular weight only used for mass density calculation, so can be constant
...
def train_validate_lolopy(
    ...
    return train_validate_sklearn(
        args, fps, data, split_idx, target_display_names, estimator=model, predict_std=True  # type: ignore
    )
